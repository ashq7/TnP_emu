...
int main(int argc, char** argv) {
    ...
    int nb_pt=9;
    float bins_pt[10]={15,21,24,30,40,50,60,120,200,500};
    TString sbins_pt[10]={"15","21","24","30","40","50","60","120","200","500"};
    int  binnum_pt = sizeof(bins_pt)/sizeof(Float_t) - 1;

    int nb_eta=4;
    float bins_eta[]={0.0,0.9,1.2,2.1,2.4};
    ...
    for (int j=0; j<nb_pt; ++j){
       for (int k=0; k<nb_eta; ++k){
          TH1F *h_pass = (TH1F*) fin->Get("pass_"+discriminant+"_etalt0p9_pt"+sbins_pt[j]+"to"+sbins_pt[j+1]);
          TH1F *h_fail = (TH1F*) fin->Get("fail_"+discriminant+"_etalt0p9_pt"+sbins_pt[j]+"to"+sbins_pt[j+1]);
	  if (k==1){
             h_pass = (TH1F*) fin->Get("pass_"+discriminant+"_eta0p9to1p2_pt"+sbins_pt[j]+"to"+sbins_pt[j+1]);
             h_fail = (TH1F*) fin->Get("fail_"+discriminant+"_eta0p9to1p2_pt"+sbins_pt[j]+"to"+sbins_pt[j+1]);
	  }
          else if (k==2){
             h_pass = (TH1F*) fin->Get("pass_"+discriminant+"_eta1p2to2p1_pt"+sbins_pt[j]+"to"+sbins_pt[j+1]);
             h_fail = (TH1F*) fin->Get("fail_"+discriminant+"_eta1p2to2p1_pt"+sbins_pt[j]+"to"+sbins_pt[j+1]);
          }
          else if (k==3){
             h_pass = (TH1F*) fin->Get("pass_"+discriminant+"_eta2p1to2p4_pt"+sbins_pt[j]+"to"+sbins_pt[j+1]);
             h_fail = (TH1F*) fin->Get("fail_"+discriminant+"_eta2p1to2p4_pt"+sbins_pt[j]+"to"+sbins_pt[j+1]);
          }
          ...
          RooRealVar mean1P("mean1P", "mean1P", 90, 85, 100);
          RooRealVar mean2P("mean2P", "mean2P", 90, 85, 100);
          RooRealVar width("width", "width", 2.495,2.495,2.495);
          RooRealVar sigma1P("sigma1P", "sigma1P", 2,0,3);
          ...
          float initial_alphaP=140.0; float initial_betaP=0.08; float initial_gammaP=0.9;
          RooRealVar alphaP("alphaP", "alphaP", 60.0, initial_alphaP, 160.0);
          RooRealVar betaP("betaP", "betaP", 0.05, initial_betaP, 0.2);
          RooRealVar gammaP("gammaP", "gammaP", 0.01, initial_gammaP, 1);
	  ...
	  RooRealVar frac1P("frac1P", "frac1P", 1000000,0.0,10000000.0);
          ...
          RooRealVar sigma1F("sigma1F", "sigma1F", 2,0,3);
          ...
          RooRealVar alphaF("alphaF", "alphaF", alphaP.getVal(), 50.0, 200.0);
          RooRealVar betaF("betaF", "betaF", betaP.getVal(), 0.01, 0.5);
          RooRealVar gammaF("gammaF", "gammaF", gammaP.getVal(), -2, 2);
	  ...
	  if (efficiency<0.5) err_efficiency=efficiency*(pow(frac1F.getError()*frac1F.getError()+frac2F.getError()*frac2F.getError(),0.5))/(frac1F.getVal()+frac2F.getVal());
  	  ...
  	  //categ.Draw("same");
          ...
          if (k==0) c->SaveAs("plots_mu_2018/fits_"+sample+"_"+discriminant+"_etalt0p9_pt"+sbins_pt[j]+"to"+sbins_pt[j+1]+".png");
          else if (k==1) c->SaveAs("plots_mu_2018/fits_"+sample+"_"+discriminant+"_eta0p9to1p2_pt"+sbins_pt[j]+"to"+sbins_pt[j+1]+".png");
          else if (k==2) c->SaveAs("plots_mu_2018/fits_"+sample+"_"+discriminant+"_eta1p2to2p1_pt"+sbins_pt[j]+"to"+sbins_pt[j+1]+".png");
          else if (k==3) c->SaveAs("plots_mu_2018/fits_"+sample+"_"+discriminant+"_eta2p1to2p4_pt"+sbins_pt[j]+"to"+sbins_pt[j+1]+".png");
          if (k==0) cout<<"plots_mu_2018/fits_"+sample+"_"+discriminant+"_etalt0p9_pt"+sbins_pt[j]+"to"+sbins_pt[j+1]+".png"<<endl;
          if (k==1) cout<<"plots_mu_2018/fits_"+sample+"_"+discriminant+"_eta0p9to1p2_pt"+sbins_pt[j]+"to"+sbins_pt[j+1]+".png"<<endl;
          if (k==2) cout<<"plots_mu_2018/fits_"+sample+"_"+discriminant+"_eta1p2to2p1_pt"+sbins_pt[j]+"to"+sbins_pt[j+1]+".png"<<endl;
          if (k==3) cout<<"plots_mu_2018/fits_"+sample+"_"+discriminant+"_eta2p1to2p4_pt"+sbins_pt[j]+"to"+sbins_pt[j+1]+".png"<<endl;
