...
class ElectronAnalyzer : public edm::one::EDAnalyzer<edm::one::SharedResources>  {
   ...
   private:
      ...
      int probe_MVAnoIsoWP90;
      int probe_MVAnoIsoWP80;
      int probe_MVAnoIsoWPLoose;
    ...
    pfToken_(consumes<pat::PackedCandidateCollection>(iConfig.getParameter<edm::InputTag>("pfCands")))
{
   ...
   tree->Branch("probe_MVAnoIsoWP90",&probe_MVAnoIsoWP90);
   tree->Branch("probe_MVAnoIsoWP80",&probe_MVAnoIsoWP80);
   tree->Branch("probe_MVAnoIsoWPLoose",&probe_MVAnoIsoWPLoose);
...
ElectronAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
    ...
    if (nElectrons==2){ 
	 ...
	 tag_dxy = (*electrons)[index_el1].gsfTrack()->dxy(reco::TrackBase::Point(pv_x, pv_y, pv_z));
      ...
      probe_MVAnoIsoWP90=(*electrons)[index_el2].electronID("mvaEleID-Fall17-noIso-V1-wp90");
      probe_MVAnoIsoWP80=(*electrons)[index_el2].electronID("mvaEleID-Fall17-noIso-V1-wp80");
      probe_MVAnoIsoWPLoose=(*electrons)[index_el2].electronID("mvaEleID-Fall17-noIso-V1-wpLoose");
